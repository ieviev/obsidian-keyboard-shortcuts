...
let rec goToPrevHeading (plugin: ExtendedPlugin<PluginSettings>) =
    Command.forEditor (nameof goToPrevHeading) "Go to previous heading" (fun editor ->
...
let rec goToNextHeading (plugin: ExtendedPlugin<PluginSettings>) =
    Command.forEditor (nameof goToNextHeading) "Go to next heading" (fun editor ->
        let cursor = editor.getCursor ()
...
let rec copyCodeBlock (plugin: ExtendedPlugin<PluginSettings>) =
    Command.forMenu (nameof copyCodeBlock) "Copy Code Block" (fun _ ->
        ...
        if codeblocks.IsNone then
            ...
            let modal =
                ...
                |> SuggestModal.withGetSuggestions (fun queryInput ->
                    ...
                    matches |> ResizeArray)
                |> SuggestModal.withRenderSuggestion (fun f elem -> elem.innerText <- f.content)
...
let rec openSwitcherWithTag1 (plugin: ExtendedPlugin<PluginSettings>) =
    Command.forMenu (nameof openSwitcherWithTag1) "Open Switcher with Tag 1" (fun _ ->
...
let rec tagSearch (plugin: ExtendedPlugin<PluginSettings>) =
    Command.forMenu (nameof tagSearch) "Search by Tag" (fun _ ->
...
let rec foldedTagSearch (plugin: ExtendedPlugin<PluginSettings>) =
    Command.forMenu (nameof foldedTagSearch) "Folded search by Tag" (fun _ ->
                    ...
                    | AddAnotherTag :: tail ->
                        ...
                        | _ ->
                            ...
                            { FoldedTagSearchState.Default with
                                ...
                                Actions = tail |> List.skipWhile (fun f -> f <> AddAnotherTag)
                            }
                            |> createModal
                    | ExpandTag :: tail ->
                        modal.close ()
            ...
            |> SuggestModal.withKeyboardShortcut
                {
                    ...
                    action =
                        (fun (evt, modal) ->
                            ...
                            let exists =
                                state.Filters |> Seq.exists (fun f -> f = current)
...
let rec insertHeading4 (plugin: ExtendedPlugin<PluginSettings>) =
    Command.forEditor (nameof insertHeading4) "Insert heading 4" (fun edit ->
        edit.replaceSelection ("#### ")
        doNone)

let rec insertHeading5 (plugin: ExtendedPlugin<PluginSettings>) =
    Command.forEditor (nameof insertHeading5) "Insert heading 5" (fun edit ->
        edit.replaceSelection ("##### ")
        doNone)

let rec increaseHeading (plugin: ExtendedPlugin<PluginSettings>) =
    Command.forEditor (nameof increaseHeading) "Increase Heading level" (fun edit ->
...
let rec decreaseHeading (plugin: ExtendedPlugin<PluginSettings>) =
    Command.forEditor (nameof decreaseHeading) "Decrease Heading level" (fun edit ->
        let lineIdx = edit.getCursor().line
        let currLine = lineIdx |> edit.getLine
...
let rec insertDefaultCallout (plugin: ExtendedPlugin<PluginSettings>) =
    Command.forEditor (nameof insertDefaultCallout) "Insert Default Callout" (fun edit ->
